...
public class KillPort {
 ...
 /*   public KillPort(HashSet ports) {
    ...
    public void check(int port) {
        this.ports.add(port);
        if (0 == port) {
            System.err.println("没有获取正确端口号，或者没有配置端口");
        }
        ...
        try {
            ...
            if (read.size() == 0) {
                ...
                System.out.println(port + "端口找到" + read.size() + "个进程，正在准备清理");
                kill(read);
                System.out.println(port + "端口已可用，继续启动程序...");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
